...
export function createApp(options = {}) {
  ...
  return createServer(async (req, res) => {
    ...
    const previewMatch = url.pathname.match(/^\/p\/([A-Za-z0-9_-]+)\/preview\.svg$/);
    if (req.method === 'GET' && previewMatch) {
      ...
      send(200, previewSvg(page, page.mime), { 'content-type': 'image/svg+xml', 'cache-control': 'public, max-age=300' });
      return;
    }
    ...
    if (pageMatch) {
      ...
      const origin = `${url.protocol}//${url.host}`;
      send(200, pageShell(page, `/p/${page.id}/content`, page.mime, `${origin}/p/${page.id}/preview.svg`), { 'content-security-policy': "default-src 'self'; frame-src 'self'; script-src 'unsafe-inline' https://cdnjs.cloudflare.com; style-src 'unsafe-inline' https://cdnjs.cloudflare.com;" });
      return;
    }
