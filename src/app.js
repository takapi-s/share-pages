...
function pageShell(page, contentUrl, contentType) {
  ...
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><meta property="og:title" content="${title}"><meta property="og:description" content="${escapeHtml(description)}"><meta name="description" content="${escapeHtml(description)}"><style>:root{color-scheme:light}*{box-sizing:border-box}body{font-family:system-ui,sans-serif;margin:0;color:#222}header{max-width:1100px;margin:0 auto;padding-left:1rem;padding-right:1rem}main{margin:0 auto;padding:2rem 1rem;line-height:1.7}header{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding-top:1rem;padding-bottom:1rem;border-bottom:1px solid #ddd}.brand{font-weight:700;color:inherit;text-decoration:none}.meta{color:#666;font-size:.9rem;margin:0}.tools{display:flex;align-items:center;gap:1rem}.download{color:inherit;white-space:nowrap}.content{width:100%}.page-layout{display:grid;grid-template-columns:minmax(0,1fr) minmax(320px,40%);gap:24px;align-items:start}.page-layout[data-mode="preview"]{grid-template-columns:1fr}.page-layout[data-mode="source"]{grid-template-columns:1fr}.page-layout[data-mode="preview"] .source-panel,.page-layout[data-mode="source"] .preview-pane{display:none}.preview-pane{max-height:70vh;overflow:auto}.source-panel{position:sticky;top:1rem;min-width:0}.view-switcher{grid-column:1/-1;display:flex;gap:.35rem;margin-bottom:.75rem}.view-switcher button{padding:.4rem .7rem;border:1px solid #bbb;border-radius:6px;background:#fff;color:#333;cursor:pointer}.view-switcher button[aria-pressed="true"]{background:#222;color:#fff;border-color:#222}.source-toolbar{display:flex;align-items:center;justify-content:space-between;gap:.75rem;margin-bottom:.5rem}.source-toolbar strong{font-size:.95rem}.source-panel textarea{display:block;width:100%;min-height:70vh;resize:vertical;padding:1rem;background:#f7f7f7;color:#222;border:1px solid #ccc;border-radius:8px;font:12px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace}.source-panel button{padding:.45rem .75rem;border:1px solid #bbb;border-radius:6px;background:#fff;cursor:pointer}.source-panel button:disabled{opacity:.5;cursor:not-allowed}.source-status{min-height:1.4em}.markdown-content{max-width:1040px;margin:0 auto;padding:0 24px 2rem}.markdown-content h1,.markdown-content h2,.markdown-content h3,.markdown-content h4{line-height:1.3;margin:1.6em 0 .6em}.markdown-content h1{font-size:2rem}.markdown-content h2{font-size:1.5rem}.markdown-content h3{font-size:1.25rem}.markdown-content hr{border:0;border-top:1px solid #d9d9d9;margin:2rem 0}.markdown-content blockquote{margin:1.25rem 0;padding:.25rem 1rem;border-left:4px solid #bbb;color:#555;background:#fafafa}.markdown-content ul,.markdown-content ol{padding-left:1.6rem}.markdown-content li+li{margin-top:.35rem}.markdown-content pre{overflow:auto;padding:1rem;background:#f6f6f6;border-radius:6px}.markdown-content table{width:100%;border-collapse:collapse;margin:1.5rem 0;font-size:.95em}.markdown-content th,.markdown-content td{border:1px solid #d9d9d9;padding:.6rem .75rem;text-align:left;vertical-align:top}.markdown-content th{background:#f6f6f6;font-weight:600}@media(max-width:600px){.page-layout{grid-template-columns:1fr}.source-panel{position:static}.markdown-content{padding-left:0;padding-right:0}}iframe{display:block;width:100%;min-height:calc(100vh - 120px);border:0}code{background:#f3f3f3;padding:.1em .3em;border-radius:4px}@media(max-width:600px){header{align-items:flex-start;flex-wrap:wrap}.tools{width:100%;justify-content:space-between}.content{padding-top:1.25rem}}</style></head><body><header><a class="brand" href="/">share-pages</a><div class="tools"><p class="meta">${escapeHtml(description)}</p><a class="download" href="/p/${page.id}/download">元ファイルをダウンロード</a></div></header><main class="page-layout" data-mode="preview"><div class="view-switcher" role="group" aria-label="表示切替"><button type="button" data-mode="split" aria-pressed="false">分割</button><button type="button" data-mode="preview" aria-pressed="true">プレビュー</button><button type="button" data-mode="source" aria-pressed="false">原文</button></div><section class="preview-pane"><div class="content">${contentType === 'text/html' ? `<iframe sandbox="allow-same-origin" src="${contentUrl}" title="${title}"></iframe>` : `<article class="markdown-content">${page.rendered}</article>`}</div></section><aside class="source-panel"><div class="source-toolbar"><strong>原文（${typeLabel}）</strong><button id="saveSource" type="button">保存</button></div><textarea id="sourceEditor" spellcheck="false">${escapeHtml(page.source || '')}</textarea><p id="sourceStatus" class="meta source-status"></p></aside></main><script>const layout=document.querySelector('.page-layout'),editor=document.querySelector('#sourceEditor'),preview=document.querySelector('.preview-pane'),iframe=preview.querySelector('iframe'),save=document.querySelector('#saveSource'),status=document.querySelector('#sourceStatus'),switches=[...document.querySelectorAll('.view-switcher button')];let syncing=false;function setMode(mode){layout.dataset.mode=mode;switches.forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.mode===mode)))}switches.forEach(button=>button.onclick=()=>setMode(button.dataset.mode));function ratio(element){return Math.max(0,element.scrollHeight-element.clientHeight)}function syncTo(from,to){if(syncing)return;const fromMax=ratio(from);if(!fromMax)return;const percent=from.scrollTop/fromMax;syncing=true;requestAnimationFrame(()=>{to.scrollTop=percent*ratio(to);syncing=false})}editor.addEventListener('scroll',()=>{if(iframe?.contentWindow)syncTo(editor,iframe.contentWindow.document.documentElement);else syncTo(editor,preview)});preview.addEventListener('scroll',()=>syncTo(preview,editor));iframe?.addEventListener('load',()=>{try{const doc=iframe.contentWindow.document;iframe.contentWindow.addEventListener('scroll',()=>syncTo(doc.documentElement,editor));doc.addEventListener('click',event=>{const element=event.target?.closest?.('*');if(!element)return;if(element.closest('a'))event.preventDefault();focusSourceElement(element)})}catch{}});function focusSourceElement(element){const tag=element.tagName?.toLowerCase();if(!tag)return;const source=editor.value;const selectors=element.id?['id="'+element.id+'"',"id='"+element.id+"'"]:[...element.classList].map(name=>'class="'+name+'"');let index=-1;for(const selector of selectors){index=source.indexOf(selector);if(index>=0)break}if(index<0)index=source.indexOf('<'+tag);if(index<0)return;editor.focus();editor.setSelectionRange(index,Math.min(source.length,index+Math.max(1,element.outerHTML?.length||tag.length+2)));const line=source.slice(0,index).split('\\n').length;editor.scrollTop=(line/Math.max(1,source.split('\\n').length))*ratio(editor);status.textContent='HTML原文の'+line+'行目へ移動'};save.onclick=async()=>{save.disabled=true;status.textContent='保存中…';const response=await fetch('/api/pages/${page.id}',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({source:editor.value})});if(response.ok){status.textContent='保存しました';location.reload()}else{status.textContent='保存に失敗しました';save.disabled=false}};</script></body></html>`;
}
