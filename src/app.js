...
function pageShell(page, contentUrl, contentType) {
  ...
  const description = `${contentType === 'text/html' ? 'HTML' : 'Markdown'}共有ページ · 期限: ${new Date(page.expiresAt).toLocaleString('ja-JP')}`;
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><meta property="og:title" content="${title}"><meta property="og:description" content="${escapeHtml(description)}"><meta name="description" content="${escapeHtml(description)}"><style>body{font-family:system-ui,sans-serif;max-width:1100px;margin:2rem auto;padding:0 1rem;color:#222}iframe{display:block;width:100%;min-height:70vh;border:1px solid #ddd;border-radius:8px}main{line-height:1.7}code{background:#f3f3f3;padding:.1em .3em;border-radius:4px}.meta{color:#666;font-size:.9rem}a.button{display:inline-block;margin:.5rem 0;padding:.4rem .7rem;border:1px solid #bbb;border-radius:5px;text-decoration:none;color:inherit}</style></head><body><header><h1>${title}</h1><p class="meta">${escapeHtml(description)}</p><a class="button" href="/p/${page.id}/download">元ファイルをダウンロード</a></header><main>${contentType === 'text/html' ? `<iframe sandbox="" src="${contentUrl}" title="${title}"></iframe>` : `<article>${page.rendered}</article>`}</main></body></html>`;
}
