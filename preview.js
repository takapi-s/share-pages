...
function png(width, height, pixels) {
  const raw = new Uint8Array(height * (1 + width * 3)); for (let y = 0; y < height; y++) { raw[y * (1 + width * 3)] = 0; raw.set(pixels.subarray(y * width * 3, (y + 1) * width * 3), y * (1 + width * 3) + 1); }
  ...
  const deflate = new Uint8Array(2 + blocks.reduce((n, b) => n + b.length, 0) + 4); deflate.set([0x78, 0x01]); let p = 2; for (const block of blocks) { deflate.set(block, p); p += block.length; } let a = 1, b = 0; for (const value of raw) { a = (a + value) % 65521; b = (b + a) % 65521; } new DataView(deflate.buffer).setUint32(p, (b << 16) | a); const header = new Uint8Array(13); new DataView(header.buffer).setUint32(0, width); new DataView(header.buffer).setUint32(4, height); header[8] = 8; header[9] = 2; const signature = new Uint8Array([137,80,78,71,13,10,26,10]); const ihdr = chunk('IHDR', header), idat = chunk('IDAT', deflate), iend = chunk('IEND', new Uint8Array()); const out = new Uint8Array(signature.length + ihdr.length + idat.length + iend.length); out.set(signature); out.set(ihdr, signature.length); out.set(idat, signature.length + ihdr.length); out.set(iend, signature.length + ihdr.length + idat.length); return out;
}
